...
namespace WrapperApi.Services
{
	public class JobService
	{
		...
		public JobService(IServiceScopeFactory scopeFactory, IWebHostEnvironment env, HeartbeatCache cache, string inputDir, string outputDir)
		{
		...
		public async Task<(bool Success, string? ErrorMessage)> RunJobAsync(Job job, string dxValue)
		{
			...
			try
			{
				...
				var process = new Process
				{
					StartInfo = new ProcessStartInfo
					{
						...
						Arguments = $"run --rm --platform {platform} --name {containerName} " +
									...
									$"{imageName} {fileName} {dxValue} {heartbeatEndpoint} {heartbeatInterval}",
				...
				_cache.MarkJobStarted(job.JobId ?? job.Id.ToString());
				...
				_cache.MarkJobCompleted(job.JobId ?? job.Id.ToString());
